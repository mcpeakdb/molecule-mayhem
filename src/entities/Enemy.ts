...
interface EnemyConfig { hp: number; speed: number; damage: number; attackRate: number; texture: string; scale: number }
...
export default class Enemy {
  ...
  update(_time: number, delta: number, playerSprite: Phaser.Physics.Arcade.Sprite): void {
    ...
    switch (this.state) {
      case STATES.PATROL: this._patrol(speed);       break;
      case STATES.CHASE:  this._chase(dx, dy, dist, speed); break;
      case STATES.ATTACK: this._tryAttack(playerSprite);    break;
    }
  ...
  takeDamage(amount: number, knockbackDir = 1, slow = false): void {
    ...
    this.sprite.body.setVelocity(knockbackDir * 200, -50);
